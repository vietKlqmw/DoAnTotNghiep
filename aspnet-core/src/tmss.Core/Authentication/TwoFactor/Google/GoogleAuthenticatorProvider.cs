...
namespace tmss.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : tmssServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("tmss", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
