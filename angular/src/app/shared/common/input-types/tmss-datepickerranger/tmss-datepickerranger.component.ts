...
import { BsDatepickerContainerComponent, BsDatepickerDirective
    ,BsDaterangepickerDirective , BsDaterangepickerContainerComponent  } from 'ngx-bootstrap/datepicker';
...
@Component({
	selector: 'tmss-datepickerranger',
	templateUrl: './tmss-datepickerranger.component.html',
	styleUrls: ['./tmss-datepickerranger.component.less'],
	providers: [
		{
			provide: NG_VALUE_ACCESSOR,
			useExisting: forwardRef(() => TmssDatepickerrangerComponent),
			multi: true
		}]
})
export class TmssDatepickerrangerComponent extends AppComponentBase implements ControlValueAccessor, OnInit {
	@ViewChild('datepickerranger', { static: false }) datepickerranger!: BsDaterangepickerDirective;
  ...
  @Input() minYear = 1970;
    ...
    focusout(){
        if (this.value === null){
            this.datepickerranger.bsValueChange.emit(this.inputValue ?? this.value);
	...
	writeValue(val: any) {
		setTimeout(()=>{
            ...
            if (!val && !this.defaultValue) return this.datepickerranger?.bsValueChange.emit(undefined);;
            ...
            this.datepickerranger?.bsValueChange.emit([dateValue, new Date()]);
        },100)
	}
	...
	hideDatePicker(event: BsDaterangepickerContainerComponent  ) {
	...
	openDatePicker() {
		...
		// this.datepickerranger._bsValue = this.value ?? new Date();
        this.datepickerranger._bsValue = [this.value ?? new Date(), new Date()];
		setTimeout(() => {
			this.datepickerranger.show();
	...
	createTimeChoosingComponent() { // Tạo 2 ô tính giờ ở cuối khi date picker được show ra
		...
		if (!this.value) this.value = this.datepickerranger._bsValue[0];
		...
		minuteControl.onkeydown = e => {
			if ((e.key === 'Tab' || e.key === 'Enter') && !e.shiftKey && !e.altKey) {
				this.value?.setHours(Number(this.hour), Number(this.minute));
				this.datepickerranger.bsValueChange.emit([new Date(this.value)]);
				this.datepickerranger.hide();
			}
		};
        ...
        hourControl.onkeyup = e => {
			if (e.key === 'Enter') {
				this.datepickerranger.bsValueChange.emit([new Date(this.value), new Date()]);
				this.datepickerranger.hide();
			};
		};
