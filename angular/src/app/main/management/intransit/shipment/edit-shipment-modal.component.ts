...
export class EditShipmentModalComponent extends AppComponentBase {
    ...
    show(type, material?: ProdShipmentDto): void {
        ...
        const dateValue2 = this.rowData.etd ? new Date(this.rowData.etd?.toString()) : new Date();
        this.datepicker2?.bsValueChange.emit(dateValue2);
        const dateValue3 = this.rowData.eta ? new Date(this.rowData.eta?.toString()) : new Date();
        this.datepicker3?.bsValueChange.emit(dateValue3);
        const dateValue4 = this.rowData.ata ? new Date(this.rowData.ata?.toString()) : new Date();
        this.datepicker4?.bsValueChange.emit(dateValue4);
        const dateValue5 = this.rowData.atd ? new Date(this.rowData.atd?.toString()) : new Date();
